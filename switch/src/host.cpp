...
static bool VideoCB(uint8_t *buf, size_t buf_size, ChiakiSeqNum16 frame_index,
		uint64_t frame_ready_time_us, int32_t frames_lost, bool frame_recovered, void *user)
{
	IO *io = (IO *)user;
	return io->VideoCB(buf, buf_size, frame_index, frame_ready_time_us, frames_lost,
			frame_recovered, user);
}
