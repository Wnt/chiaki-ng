#!/usr/bin/env bash
# PLE-404: one continuous LAN stream with `clean -> roam-1200ms -> clean` stepped under
# it, and the badge's own 1 Hz "Quality badge:" line as the series. PLE-366's harness,
# with one profile swapped and one path changed; everything else is deliberately
# identical so this capture and PLE-366/PLE-411's are read the same way.
#
# The question is narrow: `blip-200ms` is one bad second in twenty and the tail arm's
# single tier calls it CONSTRAINED. `roam-1200ms` is a *total outage* 1.2 s wide -- six
# times the duration, and dropped rather than delayed -- which is the shape a player
# calls unplayable. If the badge says the same word for both, the tail arm's single tier
# is under-reporting the worse one.
#
# Two paths, not one repo:
#   - the fork worktree (this file's own) supplies the capture guard and the analysis;
#   - WS supplies the *workspace* scripts, and it is this ticket's workspace worktree
#     rather than /home/wnt/gta6, because `roam-1200ms` does not exist in the live
#     workspace repo until this ticket lands and AGENTS.md forbids editing it in place.
#     Point WS at /home/wnt/gta6 once this has landed.
set -uo pipefail
REPO=/home/wnt/gta6
WS=${WS:-/home/wnt/gta6/wt/ple-404-ws}
...
PROFILES=${PROFILES:-"clean roam-1200ms clean"}
source "$HERE/ui.sh"
...
for p in $PROFILES; do
  ...
  elif [ "$p" = blip-200ms ] || [ "$p" = roam-1200ms ]; then
