...
#define CHECK_STOP(quit_label) do { \
	...
	err = chiaki_feedback_sender_init(&stream_connection->feedback_sender, &stream_connection->takion,
		session->connect_info.feedback_state_min_interval_ms,
		session->connect_info.feedback_stats_log_interval_ms);
	if(err != CHIAKI_ERR_SUCCESS)
	{
	...
	while(true)
	{
		...
		if(stream_stats_enabled)
		{
			...
			stats_event.stream_stats.takion_partial_frame_units_missing = packets_lost - previous_packets_lost;
